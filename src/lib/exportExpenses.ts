...
// Standard PostScript Helvetica advance widths in units of 1/1000 em.
// Indexed by char code (0-255). Used for accurate right-alignment.
...
function measureText(value: string, size: number, bold: boolean): number {
  const table = bold ? HELVETICA_BOLD_WIDTHS : HELVETICA_WIDTHS;
  const fallback = bold ? 600 : 500;
  let width = 0;
...
class PdfReport {
  ...
  private text(
    ...
    const tx = align === 'right' ? x - measureText(value, size, bold) : x;
